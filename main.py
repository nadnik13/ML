...
with open('wikibig.text', 'r') as f:
    ...
    #создаем датафрейм для сортировки
    data = pd.DataFrame()
    data['values'] = dict_word_counts.values()
    data['key'] = dict_word_counts.keys()
    data = data.sort_values(by='values', ascending=False) #сортируем
    top_words = data[:10] #создаем top-10
    #строим график
    plt.figure('words')
...
# создание словарей
month_name = {month: i for i, month in
